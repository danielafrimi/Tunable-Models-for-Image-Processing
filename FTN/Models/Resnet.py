...
#             Conv_BN_ReLU(in_channels=input_chnl, out_channels=num_chnl, kernel_size=kernel_size, padding=padding,
#                          groups=groups))
#         self.conv_layers = self._make_layers(Conv_BN_ReLU, num_chnl=num_chnl, kernel_size=kernel_size, padding=padding,
#                                              num_of_layers=num_of_layers - 2, groups=groups)
#         self.conv_out = nn.Sequential(
#             Conv_BN_ReLU(in_channels=num_chnl, out_channels=output_chnl, kernel_size=kernel_size, padding=padding,
#                          groups=groups))
#
...
class Resnet(nn.Module):
    def __init__(self, input_chnl=3):
        super(Resnet, self).__init__()
        self.conv1 = nn.Sequential(nn.Conv2d(in_channels=input_chnl, out_channels=64,
                                             kernel_size=3, stride=1, padding=1), nn.ReLU(inplace=True))

        self.dn_block = self._make_layers(Conv_BN_ReLU, 3, 64, num_of_layers=10, bias=False)

        self.output = nn.Conv2d(in_channels=64, out_channels=input_chnl,
